...
<div class="wrapper">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header with-border">
              ...
              <a href="#edit_bonus" data-toggle="modal" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-gift"></i> Edit Registration Bonus</a>
            </div>
            <div class="box-body">
  ...
  <!-- Edit Registration Bonus Modal -->
  <div class="modal fade" id="edit_bonus">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          ...
          <h4 class="modal-title"><b>Edit Registration Bonus</b></h4>
        </div>
        <div class="modal-body">
          <form class="form-horizontal" method="POST" action="registration_update.php" id="bonus_form">
            <input type="hidden" name="id" value="1">
            <div class="form-group">
              <label for="bonus_amount" class="col-sm-3 control-label">Bonus Amount</label>
              <div class="col-sm-9">
                <input type="number" class="form-control" id="bonus_amount" name="amount" required min="0" step="0.01">
              </div>
            </div>
            <div class="form-group">
              <label for="bonus_description" class="col-sm-3 control-label">Description</label>
              <div class="col-sm-9">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default btn-flat pull-left" data-dismiss="modal"><i class="fa fa-close"></i> Close</button>
          <button type="submit" class="btn btn-primary btn-flat" name="save_bonus" form="bonus_form"><i class="fa fa-save"></i> Save</button>
        </div>
      </div>
    </div>
  </div>
...
$(function(){
  ...
  // Load current bonus values when the Edit Registration Bonus modal is opened
  $(document).on('click', '[href="#edit_bonus"]', function(e){
    e.preventDefault();
    $('#edit_bonus').modal('show');
    $.ajax({
      type: 'POST',
      url: 'registration_row.php',
      data: {id: 1},
      dataType: 'json',
      success: function(response){
        $('#bonus_amount').val(response.amount);
        $('#bonus_description').val(response.description);
      },
      error: function(){
        alert('Error loading bonus details');
      }
    });
  });
